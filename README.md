...
- `BinaryFormatter.exe`: <https://github.com/SqlQuantumLeap/BinaryFormatter/releases>
- `makecert.exe`, `pvk2pfx.exe`, `signtool.exe` from the Windows SDK (e.g. `C:\Program Files (x86)\Windows Kits\10\bin\10.0.17763.0\x86`).
...
2. to merge `.cer` and `.pvk` files into a `.pfx` file: in a Command Prompt window:
...
4. to convert the public key for VARBINARY literal (get `BinaryFormatter.exe` from <https://github.com/SqlQuantumLeap/BinaryFormatter/releases>):
...
1. in main Project, go to Project Properties | SQLCLR tab | Signing… button; check the `Sign the assembly` check-box, `Browse…` in `Choose a strong name key file`, find the KeyAsm PFX in the fake project folder and select it.
...
2. unregister the existing UDF and their assembly if updating them:
...
1. execute `Predeploy.sql`
2. execute `Regex.sql` (if updating, first unregister as explained above)
